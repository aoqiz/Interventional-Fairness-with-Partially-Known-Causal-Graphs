...
def simulate_data(dv_c_ls, B, n):
    ...
    def _simulate_single_equation(X, B_i, N_i, ffun_type, gfun_type):
        ...
        # g() function type
        pa_size = X.shape[1] # the size of the parsents 

        if gfun_type == 0: # Linear
            X_i = X_i @ B_i + N_i

        elif gfun_type == 1: # MLP function
            hidden = 100
            W1 = np.random.uniform(low=0.5, high=2.0, size=[pa_size, hidden])
            W1[np.random.rand(*W1.shape) < 0.5] *= -1
            W2 = np.random.uniform(low=0.5, high=2.0, size=hidden)
            W2[np.random.rand(hidden) < 0.5] *= -1
            X_i = sigmoid(X_i @ W1) @ W2 + N_i

        elif gfun_type == 2: #MiM function
            ...
            X_i = np.tanh(X_i @ w1) + np.cos(X_i @ w2) + np.sin(X_i @ w3)+ N_i

        elif gfun_type == 3: # GP function
            gp = GaussianProcessRegressor()
            X_i = gp.sample_y(X_i, random_state=None).flatten() + N_i

        elif gfun_type == 4: # GP-add function
            gp = GaussianProcessRegressor()
            X_i = sum([gp.sample_y(X_i[:, i, None], random_state=None).flatten()
                     for i in range(X_i.shape[1])]) + N_i
        else:
    ...
    def _generate_noise(noise_type=0):
        v_noise = 1.0
    ...
    for i in ordered_vertices:
        ...
        # print("Node {}: ".format(i))
        gfun_type = np.random.randint(4, size=1)[0]
        noise_type = np.random.randint(4, size=1)[0]
        noise_matrix[:, i] = _generate_noise(noise_type)
