...
def save_results(args, RMSE, Unfairness, Accuracy, name="", lambdas=[]):
    results_dir = "results"
    if (not os.path.exists("{}/RMSE{}.csv".format(results_dir, name))) or (not os.path.exists("{}/Unfairness{}.csv".format(results_dir, name))):
        ...
        pd.DataFrame(pd_header).to_csv("{}/Accuracy{}.csv".format(results_dir, name), index=False, header=True)
    ...
    pd.DataFrame(Accuracy).to_csv("{}/Accuracy{}.csv".format(results_dir, name), mode='a', index=False, header=False)
