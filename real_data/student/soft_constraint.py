...
# main function
def main(args):
    args.writer = SummaryWriter("student.log")

    # ------Parameters setting------
    set_random_seed(args.seed)
    seeds_to_split_data = np.random.randint(low=0, high=9999, size=20)
    ...
    args.protected = 'sex'
    args.outcome = 'Grade'
    args.Des = ['Walc', 'Dalc', 'studytime', 'goout']    # numerical
    args.NonDes = list(set(datasets['observation'].columns) - set([args.protected, args.outcome] + ['failure'] + args.Des))  # 去掉failure
    ...
    datasets['interventional1_truth'] = pd.concat(
                [datasets['interventional1_truth'], datasets['interventional1_truth'].sample(n0_real - n1_real)], 
                ignore_index=True)
    ...
    for i, seed in enumerate(seeds_to_split_data):
        ...
        for i, mode in enumerate(args.mode_var.keys()):
            ...
            train(args, model, datasets, optimizer, num_iters=ITER, eval_every=1, lambda_=lambda_, mode=mode)
            # Predict
            ...
            # pd.DataFrame(y0_test_pred.view(1,-1).numpy()).to_csv("{}/{}/{}_y0.csv".format(results_dir, y_test_pred_dir, mode), mode='a', index=False,
            #                                   header=False)
            # pd.DataFrame(y1_test_pred.view(1,-1).numpy()).to_csv("{}/{}/{}_y1.csv".format(results_dir, y_test_pred_dir, mode), mode='a', index=False,
            #                                   header=False)
        ...
        #################################
        lambdas = [0, 1, 40, 100, 130, 175, 250]
        mode = 'e-IFair'
        RMSEIF = np.zeros([1, len(lambdas)])
        UnfairnessIF = np.zeros([1, len(lambdas)])
        for i, lambda_ in enumerate(lambdas):
            ITER = args.BASE_ITER if lambda_ < 10 else args.GENE_ITER
            set_random_seed(seed)
            RMSEIF_cv = []
            UnfairnessIF_cv = []
            ...
            # pd.DataFrame(y0_test_pred.view(1, -1).numpy()).to_csv("{}/{}/{}_{}_y0.csv".format(results_dir, y_test_pred_dir, mode, lambda_), mode='a',
            #                                               index=False,
            #                                               header=False)
            # pd.DataFrame(y1_test_pred.view(1, -1).numpy()).to_csv("{}/{}/{}_{}_y1.csv".format(results_dir, y_test_pred_dir, mode, lambda_), mode='a',
            #                                               index=False,
            #                                               header=False)
